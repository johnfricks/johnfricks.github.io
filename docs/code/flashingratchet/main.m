...
param(2)=9e2;        %diffusion coeff of motor
param(3)=9e2;        %diffusion coeff of cargo
param(4)=1;          %number of y mesh points in a period
param(5)=50;         %number of x values in L
param(6)=3;          %number of periods y goes through
param(7)=2;          %number of periods the mesh for y is calculated beyond origin (in positive direction) 
param(8)=0;          %spring constant K 
param(9)=10;         %positive forcing on motor
param(10)=8*param(1);    %values for amplitude 
param(11)=8;             %period of the motor
param(12)=1;             %transition  rate from state 1 to state 2
param(13)=1;             %transition from state 2 to state 1

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is how "asymptotic()" handles the param().
